...
HAVING count(q.QuestionID) = (SELECT MAX(countQues)
FROM (SELECT COUNT(*) AS countQues FROM examquestion
GROUP BY QuestionID) AS counttable);
...
SELECT q.QuestionID,q.Content,count(e.QuestionID),group_concat(e.QuestionID)
...
(SELECT count(ac.PositionID) AS ca FROM position p LEFT JOIN `account` ac ON p.PositionID=ac.PositionID 
GROUP BY p.PositionID) AS cp);
-- Question 11: Thống kê mỗi phòng ban có bao nhiêu dev, test, scrum master, PM
SELECT a.DepartmentID,d.DepartmentName,count(a.AccountID),group_concat(p.PositionName)
...
SELECT d.DepartmentName,p.PositionName,COUNT(a.AccountID) as slnv,group_concat(p.PositionName) as vitri
FROM department d
